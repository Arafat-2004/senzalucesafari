...
export function FavouriteBadge({ className }: FavouriteBadgeProps) {
    ...
    return (
        ...
        >
            ...
            {hydrated && count > 0 && (
                <span className="absolute -right-0.5 -top-0.5 flex h-4.5 min-h-[18px] w-4.5 min-w-[18px] items-center justify-center rounded-full bg-destructive text-[10px] font-bold leading-none text-destructive-foreground">
                    {count > 9 ? "9+" : count}
                </span>
            )}
        </Link>
