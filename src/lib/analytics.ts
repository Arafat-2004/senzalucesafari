...
type BookingGroupResult = { tourId: string; _count: { id: number }; _sum: { totalPrice: number | null } };
type TourDestinationGroupResult = { destinationId: string; _count: { id: number } };
type PageViewGroupResult = { device: string; _count: number };

async function groupByInquiryType() {
    const result = await prisma.$queryRaw<Array<{ inquiryType: string; _count: bigint }>>`
        SELECT "inquiryType", COUNT(*) as _count
        FROM contact_inquiries
        GROUP BY "inquiryType"
    `;
    return result.map(r => ({ inquiryType: r.inquiryType, _count: Number(r._count) })) as InquiryTypeResult[];
}

async function groupByBookingTour(limit: number) {
    const result = await prisma.$queryRaw<Array<{ tourId: string; id_count: bigint; totalPrice_sum: number | null }>>`
...
async function groupByTourDestination(limit: number) {
    const result = await prisma.$queryRaw<Array<{ destinationId: string; id_count: bigint }>>`
...
async function groupByPageViewDevice() {
    const result = await prisma.$queryRaw<Array<{ device: string; _count: bigint }>>`
...
export async function getDashboardStatsFast(months = 6, dateRange?: DateRangeFilter): Promise<DashboardStats> {
    const sixMonthsAgo = dateRange?.startDate || (() => {
        const d = new Date();
    ...
    const [
        tourCount, destinationCount, accommodationCount, vehicleCount,
        blogCount, reviewCount, bookingCount, inquiryCount,
        newsletterCount, faqCount, guideCount
    ] = countsResult;

    const [pendingBookings, unreadInquiries, pendingReviews] = alertsResult;

    const revenue: Record<string, { revenue: number; deposits: number; count: number }> = {};
    ...
    const topTours: { name: string; count: number }[] = topToursResult.map((b: any) => ({
        name: b.tour?.name || 'Unknown',
        count: b.count as number,
    }));

    const inquiriesByType: { inquiryType: string; count: number }[] = inquiryTypesResult.map((r: any) => ({
...
export async function getBookingsByMonth(months = 12, dateRange?: DateRangeFilter) {
...
export async function getRevenueByMonth(months = 12, dateRange?: DateRangeFilter) {
    const startDate = dateRange?.startDate || (() => {
        const d = new Date();
        d.setMonth(d.getMonth() - months);
        return d;
    ...
    const monthlyData: Record<string, { revenue: number; deposits: number; count: number }> = {};
...
export async function getTopToursByBookings(limit = 10) {
    ...
    const tours = await prisma.tour.findMany({
        where: { id: { in: bookings.map(b => b.tourId).filter(Boolean) as string[] } },
...
export async function getTopDestinations(limit = 10) {
    ...
    const destinations = await prisma.destination.findMany({
        where: { id: { in: tourDestinations.map(d => d.destinationId).filter(Boolean) as string[] } },
        select: { id: true, name: true, slug: true }
    });

    return tourDestinations.map(d => {
        const destination = destinations.find(dest => dest.id === d.destinationId);
        return {
            destination,
            count: d._count.id,
        };
...
export async function getBookingsByStatus() {
    const statuses = await prisma.$queryRaw<Array<{ status: string; count: bigint }>>`
...
export async function getCustomerGrowth(months = 12, dateRange?: DateRangeFilter) {
    const startDate = dateRange?.startDate || (() => {
        const d = new Date();
        d.setMonth(d.getMonth() - months);
        return d;
...
export async function getConversionStats() {
    ...
    const [inquiries, bookings, confirmedBookings] = await Promise.all([
    ...
    const conversionRate = inquiries > 0 ? (confirmedBookings / inquiries * 100).toFixed(1) : '0';
...
export async function getCurrentMonthRevenue(dateRange?: DateRangeFilter) {
    const startOfMonth = dateRange?.startDate || new Date(new Date().getFullYear(), new Date().getMonth(), 1);
...
export async function getKpiTrends() {
    ...
    const [
        ...
        prisma.booking.count({ where: { createdAt: { gte: startOfLastMonth, lt: startOfCurrentMonth } } }),
        prisma.booking.aggregate({ _sum: { totalPrice: true }, where: { createdAt: { gte: startOfCurrentMonth } } }),
        prisma.booking.aggregate({ _sum: { totalPrice: true }, where: { createdAt: { gte: startOfLastMonth, lt: startOfCurrentMonth } } }),
        prisma.booking.count({ where: { createdAt: { gte: startOfCurrentMonth } } }),
        prisma.booking.count({ where: { createdAt: { gte: startOfLastMonth, lt: startOfCurrentMonth } } }),
        prisma.contactInquiry.count({ where: { createdAt: { gte: startOfCurrentMonth } } }),
        prisma.contactInquiry.count({ where: { createdAt: { gte: startOfLastMonth, lt: startOfCurrentMonth } } }),
        prisma.blogPost.count({ where: { createdAt: { gte: startOfCurrentMonth } } }),
        prisma.blogPost.count({ where: { createdAt: { gte: startOfLastMonth, lt: startOfCurrentMonth } } }),
    ]);

    function calcTrend(thisVal: number, lastVal: number): { value: number | string; direction: "up" | "down" | "neutral" } {
        if (lastVal === 0) {
            return thisVal > 0 ? { value: "New", direction: "up" } : { value: 0, direction: "neutral" };
        }
    ...
    return {
        ...
        revenue: calcTrend(revenueThisMonth._sum.totalPrice || 0, revenueLastMonth._sum.totalPrice || 0),
