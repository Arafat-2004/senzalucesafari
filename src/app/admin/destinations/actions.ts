...
export async function createDestination(formData: FormData) {
    ...
    try {
        ...
        logCmsAction('destination', 'create', { entityId: newDestination.id, newValue: validated, userId: admin.id })
...
export async function updateDestination(id: string, formData: FormData) {
    ...
    try {
        ...
        const existing = await prisma.destination.findFirst({ where: { slug, NOT: { id } } })
        if (existing) {
            throw new Error(`Destination with slug "${slug}" already exists`)
        }
        
        const currentDestination = await prisma.destination.findUnique({ where: { id } })
        ...
        if (currentDestination) {
            logCmsAction('destination', 'update', { entityId: id, previousValue: currentDestination, newValue: validated, userId: admin.id })
        }
        invalidateDestinations()
