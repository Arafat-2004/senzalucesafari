...
import { Calendar, User, Clock, ArrowRight, Camera, MapPin, Lightbulb, AlertCircle, Check, Star, Trophy, Mountain, Waves, Building, BookOpen } from "lucide-react";
import { Button } from "@/components/ui/button";
import { Breadcrumb } from "@/components/ui/breadcrumb-nav";
import { HeroSection } from "@/components/ui/hero-section";
import { blogArticles, getAllBlogSlugs, BlogSection } from "@/data/blogs";
...
const SectionIcon = ({ type }: { type: string }) => {
    ...
    return <IconComponent className="w-7 h-7 sm:w-8 sm:h-8 text-primary flex-shrink-0 mt-1" />;
};
...
const renderSection = (section: BlogSection, index: number) => {
    switch (section.type) {
            ...
            return (
                <div key={index} className="prose prose-lg max-w-none mb-12 sm:mb-16">
                    <p className="text-xl sm:text-2xl leading-relaxed text-foreground font-light italic border-l-4 border-primary pl-6 py-2">
                        &ldquo;{section.content.text.split('\n\n')[0]}&rdquo;
                    </p>
        ...
        case 'heading':
            return (
                <h2 key={index} className={`text-2xl sm:text-3xl md:text-4xl font-bold text-foreground mb-6 ${section.content.level === 3 ? 'mt-8' : ''}`}>
            ...
            return (
                <div key={index} className="bg-primary/5 border-l-4 border-primary p-6 rounded-r-lg my-8">
                    <p className="text-base sm:text-lg text-foreground font-medium flex items-start gap-2">
                        ...
                        <span><strong>{section.content.title}:</strong> {section.content.text}</span>
                    </p>
                </div>
            );
        ...
        case 'list':
            return (
                <div key={index} className="my-8">
                    <h3 className="text-xl sm:text-2xl font-bold text-foreground mb-4">{section.content.title}</h3>
            ...
            return (
                <section key={index} className="my-12 sm:my-16 p-8 sm:p-10 md:p-12 bg-gradient-to-br from-primary/10 via-primary/5 to-background rounded-3xl border border-primary/20 text-center">
                    ...
                    <div className="flex flex-col sm:flex-row gap-4 justify-center">
                        <Button size="lg" className="btn-safari text-base sm:text-lg px-8 py-6 h-auto shadow-lg hover:shadow-xl transition-all">
                            <Link href={section.content.primaryButton.link} className="inline-flex items-center font-semibold">
                                {section.content.primaryButton.text}
                                <ArrowRight className="ml-2 w-5 h-5" />
                            </Link>
                        </Button>
                        <Button size="lg" variant="outline" className="btn-outline text-base sm:text-lg px-8 py-6 h-auto">
                            <Link href={section.content.secondaryButton.link} className="inline-flex items-center">
...
// Render grid layouts
const renderGrid = (section: any, index: number) => {
    const { columns, items } = section.content;
...
const renderTimeline = (section: any, index: number) => {
